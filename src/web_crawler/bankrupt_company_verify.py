...
def get_isca(url, headers, payload):
    ...
    for li_tag in ul_li_list:
        ...
        if "破产" in S:
            po1 = S.find("裁定受理")
            po2 = S.find("公司破产清算")
            company_name = S[po1 + 4:po2 + 2].strip(" ")
            if po1 < 0 or po2 < 0:
                po1 = S.find("以")
                po2 = S.find("公司不能清偿到期债务")
                company_name = S[po1 + 1:po2 + 2].replace('\t', '').replace('\n', '').replace(' ', '')
                if po1 < 0 or po2 < 0:
                    po1 = S.find("裁定受理")
                    po2 = S.find("公司的破产申请")
                    ...
                    if po1 < 0 or po2 < 0:
                        po1 = S.find("受理")
                        po2 = S.find("公司破产清算")
                        company_name = S[po1 + 2:po2 + 2].replace('\t', '').replace('\n', '').replace(' ', '')
                        if po1 < 0 or po2 < 0:
                            po1 = S.find("被执行人")
                            po2 = S.find("公司")
                            company_name = S[po1 + 4:po2 + 2].replace('\t', '').replace('\n', '').replace(' ', '')
...
def get_bankrupt_company_name(company_name, wslx, pageNum):
    """
...
if __name__ == '__main__':
    ...
    for company_name in bankrupt_company_list01:
        i += 1
        companys = []
        for j in range(5):
            companys = get_bankrupt_company_name(company_name=company_name, wslx=1, pageNum=j)
            companys += get_bankrupt_company_name(company_name=company_name, wslx=4, pageNum=j)
